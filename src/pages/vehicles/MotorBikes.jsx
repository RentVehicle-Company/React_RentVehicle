...
const MotorBikes = () => {
  ...
  const filtered = useMemo(() => {
    ...
    if (pickupDate || returnDate) {
      ...
      list = list.filter((moto) => {
        if (
          !Array.isArray(moto.bookedDates) ||
          moto.bookedDates.length === 0
        ) {
          return true;
        }
        return !moto.bookedDates.some((date) => {
    ...
    if (q) {
      list = list.filter((moto) =>
        `${moto.brand} ${moto.model} ${moto.category}`
          .toLowerCase()
          .includes(q)
      );
    }
  ...
  const pageMotorbikes = filtered.slice(pageStart, pageEnd);
  ...
  const renderFilters = () => (
    <div className="space-y-6">
      <div>
        <label htmlFor="moto-search" className={labelClass}>
          Search motorbike
        </label>
      ...
      <div>
        <label htmlFor="moto-location" className={labelClass}>
          Location
        </label>
        <select
          id="moto-location"
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-3">
            ...
            <div className="ml-auto flex items-center gap-3">
              <label htmlFor="moto-sort" className="sr-only">
                Sort by
              </label>
              <select
                id="moto-sort"
