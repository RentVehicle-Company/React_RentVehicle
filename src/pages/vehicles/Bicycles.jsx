...
const Bicycles = () => {
  ...
  // Backend reference data: bicycle categories + pickup locations.
  const [categoryOptions, setCategoryOptions] = useState([]);
  const [locationOptions, setLocationOptions] = useState([]);

  useEffect(() => {
    let alive = true;
    getCatalogCategories({ vehicleType: "bicycle" }).then((list) => {
      if (!alive) return;
      setCategoryOptions(list);
      const legacy = searchParams.get("category");
      if (legacy) {
        const match = list.find((cat) => categoryMatches(cat.name, legacy));
  ...
  const filtered = useMemo(() => {
    ...
    if (categories.length > 0) {
      ...
      list = list.filter((bike) => {
        if (
          bike.categoryId != null &&
          wantedIds.includes(Number(bike.categoryId))
        ) {
          return true;
        }
        return names.length > 0 && names.some((name) => categoryMatches(name, bike.category));
      });
    }
    if (location) {
      ...
      list = list.filter((bike) => {
        if (locId != null && bike.locationId != null) {
          return Number(bike.locationId) === Number(locId);
        }
        return locName
          ? String(bike.location || "").toLowerCase().includes(locName)
          : true;
      });
    }
    if (pickupDate || returnDate) {
      ...
      list = list.filter((bike) => {
        if (
          !Array.isArray(bike.bookedDates) ||
          bike.bookedDates.length === 0
        ) {
          return true;
        }
        return !bike.bookedDates.some((date) => {
    ...
    if (maxPrice < PRICE_MAX) {
      list = list.filter((bike) => bike.price_per_day <= maxPrice);
    }
  ...
  const pageBicycles = filtered.slice(pageStart, pageEnd);
  ...
  const renderFilters = () => (
    <div className="space-y-6">
      <div>
        <label htmlFor="bike-search" className={labelClass}>
          Search bicycle
        </label>
      ...
      <div>
        <label htmlFor="bike-location" className={labelClass}>
          Location
        </label>
        <select
          id="bike-location"
    ...
    <div className="max-w-[1200px] mx-auto px-4 sm:px-6 py-6 sm:py-8">
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          Mountain, road, hybrid and electric bikes for the whole family.
        </p>
      </div>
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-3">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              ...
              {filtered.length === 1 ? "Bicycle" : "Bicycles"}
            </p>

            <div className="ml-auto flex items-center gap-3">
              <label htmlFor="bike-sort" className="sr-only">
                Sort by
              </label>
              <select
                id="bike-sort"
          ...
          <div className="mt-6">
            {loading ? (
              <p className="text-sm text-slate-500 dark:text-slate-400">Loading bicycles...</p>
            ) : filtered.length > 0 ? (
              <>
                <motion.div
                  layout
